...
static void run_one_batch(
    ...
    if (batch_logits.empty()) {
        throw std::runtime_error(
            "TensorRT batch output logits is empty."
        );
    }
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        std::cout << "[INFO] TensorRT C++ benchmark started."
                  << std::endl;
        ...
        std::unique_ptr<TrtRunner> runner;

        {
            scopedTimer t(init_timer, "engine_load");

            runner = std::make_unique<TrtRunner>(engine_path);
        }

        std::cout << "[INFO] TensorRT engine loaded | cost: "
                  << init_timer.get("engine_load")
                  << " ms"
                  << std::endl;
        ...
        std::cout << std::endl;
        std::cout << "========== TensorRT Repeat Benchmark =========="
                  << std::endl;
        ...
        std::cout << "[INFO] TensorRT C++ benchmark finished."
                  << std::endl;
    }
    catch (const std::exception& e) {
        std::cerr << "[ERROR] "
